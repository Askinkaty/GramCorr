...
#    echo "_MAX_PHRASE_LENGTH_=1" >> $TRAIN_DIR/fold${i}/ActualParameters.cfg # 1-gram
    ...
    cd $TRAIN_DIR/fold${i}
    tar -czvf train-${CORPUS}fold${i}-de-en.tar.gz train.en train.de ActualParameters.cfg
    cp train-${CORPUS}fold${i}-de-en.tar.gz $CORPUS_DIR
    cd $CORPUS_DIR
    touch train-${CORPUS}fold${i}-de-en.ready
done
...
do
    echo ${i}
    while ! test -f "train-${CORPUS}fold${i}-de-en.done"
    do
        sleep 60
        echo "Still waiting"
    done
    mkdir -p $MY_MODEL_DIR/fold${i}/
    mv -f $MODELS_DIR/MODELS-${CORPUS}fold${i}-de-en.tar.gz $MY_MODEL_DIR/fold${i}
    rm -f $MODELS_DIR/MODELS-${CORPUS}fold${i}-de-en.done
    rm -f $CORPUS_DIR/train-${CORPUS}fold${i}-de-en.done
    rm -f $CORPUS_DIR/train-${CORPUS}fold${i}-de-en.tar.gz
    cd $MY_MODEL_DIR/fold${i}/
    tar -xzvf MODELS-${CORPUS}fold${i}-de-en.tar.gz
    cp $MY_MODEL_DIR/fold${i}/MODELS-${CORPUS}fold${i}-de-en/moses.ini $TUNE_DIR/fold${i}
    cd $CORPUS_DIR
done
